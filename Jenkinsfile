...
// which are part of the genix repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="genix-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /genix-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "genix-project-genix/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from genix-project-genix/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'genix-project-genix/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from genix-project-genix/develop"
            }
          }

          if (hasCache) {
            sh "cd /genix-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /genix-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /genix-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /genix-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /genix-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /genix-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /genix-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
