...
###   This script attempts to download the signature file SHA256SUMS.asc from genix.org
...
WORKINGDIR="/tmp/genix"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://genix.org/bin/"
VERSIONPREFIX="genix-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'genix-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the GENIX Core binary release signing key installed?"
   fi
