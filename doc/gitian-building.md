...
- [Building genix Core](#building-genix-core)
...
git clone https://github.com/genixpay/genix
...
in the genix Core repository under 'Fetch and create inputs' to install sources which require
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/genix/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/genixpay/genix
...
./bin/gbuild --commit genix=${COMMIT} --url genix=${URL} ../genix/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the genix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../genix/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/genixpay/genix-detached-sigs.git

BTCPATH=/some/root/path/genix
SIGPATH=/some/root/path/genix-detached-sigs

./bin/gbuild --url genix=${BTCPATH},signature=${SIGPATH} ../genix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/genix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/genix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/genix-osx-build.assert
```
...
[genix/gitian.sigs](https://github.com/genixpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
