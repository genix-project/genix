...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the genix: URI contains a payment request, we are not able to detect the
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click genix: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start genix: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling genix: URIs and PaymentRequest mime types.
