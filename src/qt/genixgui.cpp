...
const std::string GENIXGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString GENIXGUI::DEFAULT_WALLET = "~Default";

GENIXGUI::GENIXGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/genix_logo_horizontal");
...
void GENIXGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a GENIX address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and genix: URIs)"));
    ...
    aboutAction = new QAction(tr("&About GENIX"), this);
    aboutAction->setStatusTip(tr("Show information about GENIX"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for GENIX"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your GENIX addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified GENIX addresses"));
    ...
    openAction->setStatusTip(tr("Open a genix: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the GENIX help message to get a list with possible GENIX command-line options"));
...
void GENIXGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void GENIXGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool GENIXGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool GENIXGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void GENIXGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GENIXGUI::setWalletActionsEnabled(bool enabled)
{
...
void GENIXGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("GENIX client") + " " + networkStyle->getTitleAddText();
...
void GENIXGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void GENIXGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void GENIXGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GENIXGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GENIXGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void GENIXGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void GENIXGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void GENIXGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GENIXGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GENIXGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GENIXGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void GENIXGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to GENIX network", "", count));
    } else {
...
void GENIXGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GENIXGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void GENIXGUI::updateHeadersSyncProgressLabel()
{
...
void GENIXGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void GENIXGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void GENIXGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("GENIX"); // default title
...
void GENIXGUI::changeEvent(QEvent *e)
{
...
void GENIXGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GENIXGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GENIXGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(GENIXUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void GENIXGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GENIXGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GENIXGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GENIXGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GENIXGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void GENIXGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void GENIXGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GENIXGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GENIXGUI::detectShutdown()
{
...
void GENIXGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void GENIXGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void GENIXGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GENIXGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void GENIXGUI::subscribeToCoreSignals()
{
...
void GENIXGUI::unsubscribeFromCoreSignals()
{
...
void GENIXGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void GENIXGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<GENIXUnits::Unit> units = GENIXUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const GENIXUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(GENIXUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(GENIXUnits::Unit u, GENIXUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(GENIXUnits::name(u)), this);
...
{
    setText(GENIXUnits::name(newUnits));
}
