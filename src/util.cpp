...
const char * const GENIX_CONF_FILENAME = "genix.conf";
const char * const GENIX_PID_FILENAME = "genixd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "genix" is a composite category enabling all GENIX-related debug output
            if(ptrCategory->count(string("genix"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GENIXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GENIXCore
    // Mac: ~/Library/Application Support/GENIXCore
    // Unix: ~/.genixcore
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", GENIX_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty genix.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override genix.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", GENIX_PID_FILENAME));
