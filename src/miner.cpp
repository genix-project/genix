...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in genix, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in GENIX
//
...
// ***TODO*** that part changed in genix, we are using a mix with old one here for now
void static GENIXMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("GENIXMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("genix-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("GENIXMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("GENIXMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("GENIXMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("GENIXMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateGENIXs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GENIXMiner, boost::cref(chainparams), boost::ref(connman)));
}
