...
# genix_hash
RUN git clone https://github.com/genix-project/genix_hash
RUN cd genix_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} genix
RUN useradd -u ${USER_ID} -g genix -s /bin/bash -m -d /genix genix
...
RUN mkdir /genix-src && \
  ...
  chown $USER_ID:$GROUP_ID /genix-src && \
